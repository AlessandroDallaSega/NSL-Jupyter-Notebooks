...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double prob_plus = 1/(1 + exp(- (2*_J*_beta) * (_particle[this->pbc(i-1)].getspin() + _particle[this->pbc(i+1)].getspin())-2*_beta*_H));
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/potential_energy_solid_100000.dat");
        ...
        index_property++;
        _vtail = (8.0*M_PI*_rho)/(9.0*pow(_r_cut, 9)) - (8.0*M_PI*_rho)/(3.0*pow(_r_cut, 3));
        ...
        ofstream coutpr("../OUTPUT/pressure_solid_100000.dat");
        ...
        index_property++;
        _ptail = (32.0*M_PI*_rho)/(9.0*pow(_r_cut, 9)) - (16.0*M_PI*_rho)/(3.0*pow(_r_cut, 3));
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr_gas_NVE.dat");
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (dr < _halfside.min()){
          distance_gofr.push_back(dr);
        }
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          if(_measure_pressure) pressure_temp += 48*(1.0/pow(dr, 12) - 0.5/pow(dr, 6)); //PRESSURE (sigma e epsilon uguali a uno)
        }
      }
    }
  }
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    vector<double> histogram(_n_bins, 0);
    for(double r : distance_gofr){
      int bin = static_cast<int>(r / _bin_size);
      if (bin < _n_bins) {
        histogram[bin]++;
      }
    }
    for (int i = 0; i < _n_bins; i++){
      double r_inner = i * _bin_size;
      double r_outer = (i + 1) * _bin_size;
      double shell_volume = (4.0 / 3.0) * M_PI * (pow(r_outer, 3) - pow(r_inner, 3));
      double ideal_gas = _rho * shell_volume;
      _measurement(_index_gofr + i) = histogram[i] / ideal_gas;
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure and _measure_temp) _measurement(_index_pressure) = (_rho*_measurement(_index_temp) + (1/(3*_volume))*pressure_temp);
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    for (int i=0; i<_npart; i++){
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_tenergy and _measure_cv) _measurement(_index_cv) = pow(_measurement(_index_tenergy)*double(_npart), 2);
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    magnetization = 0;
    for (int i=0; i<_npart; i++){
    ...
    _measurement(_index_chi) = pow(magnetization, 2)*_beta;
  }
...
void System :: averages(int blk){
  ...
  if (_measure_cv){
    _average(_index_cv) = pow(_beta, 2) * (_block_av(_index_cv)/double(_nsteps) - pow(_average(_index_tenergy)*double(_npart), 2));
  }
  _global_av  += _average;
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/potential_energy_solid_100000.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open("../OUTPUT/pressure_solid_100000.dat",ios::app);
  ...
  if (_measure_gofr){
    for (int i = 0; i < _n_bins; i++){
      ...
      if (blk == _nblocks){
        coutf.open("../OUTPUT/gofr_gas_NVE.dat",ios::app);
        coutf << setw(12) << (i + 0.5) * (_halfside.min() / _n_bins)
              << setw(12) << sum_average/double(blk)
              << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
        coutf.close(); 
      }
